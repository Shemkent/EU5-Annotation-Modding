...
Do **not** use the same key for both the effect and its `custom_description`. The engine can't distinguish them:
```
# BAD:
my_effect = { custom_description = { text = my_effect  object = ... } }

# GOOD:
my_effect = { custom_description = { text = my_effect_text  object = ... } }
```
