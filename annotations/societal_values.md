...
| `sinicized_vs_unsinicized` | Sinicized (governance, trade) | Unsinicized (prestige, stability) | `allow`: non-Chinese near China or in Middle Kingdom IO |
...
| `monthly_towards_<left/right>` | Modifier key used elsewhere to move this slider | Used in traits, employment systems, laws, events to push the slider monthly |
...
- **`monthly_towards_*` modifier keys** are auto-generated from the value ID: the left side is `monthly_towards_<left_name>` and the right side `monthly_towards_<right_name>`. For `centralization_vs_decentralization`, the keys are `monthly_towards_centralization` and `monthly_towards_decentralization`. Verify these keys against the actual modifier definitions or by reading the employment systems source.
...
- **`opinion_importance_multiplier`** is used by the AI antagonism system. A value of 0 (e.g., `land_vs_naval`) means divergence on this axis does not affect AI opinions. Values of 0.5 (e.g., `centralization_vs_decentralization`) create moderate opinion effects. Most values omit this field, implying a game default.
